from typing import Optional, Callable, Union, Iterable, Dict, List, Text
...
def stream_json(filename: str) -> Iterable[Dict]:
    """
    Parses a json file and yields it as a dictionary
    """
    with open(filename, "r") as fp:
        data = json.load(fp)
        for x in data:
...
def combine_results(sample_file, results):
    
    data = read_json(sample_file)
    for problem in data:
        result = results[problem["problem_id"]]
        correctness = [r[1]["passed"] for r in result]
...
def check_correctness(problem: Dict, completion: str, timeout: float,
    ...
    def unsafe_execute():
        ...
        with create_tempdir():
            ...
            if "def " + problem["entry_point"] in completion:
                # completion returns the entire function
                check_program = (
                    completion + "\n" +
                    problem["test"] + "\n" +
                ...
                # completion returns the part of the program that goes inside the function
                check_program = (
